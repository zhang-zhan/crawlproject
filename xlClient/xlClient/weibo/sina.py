...
def _parse_json(s, parseJson=True):
    if not parseJson:
        return s
...
def _http_call(the_url, method, authorization, parseJson=True, **kw):
    '''
...
class APIClient(object):
    ...
    def __init__(self, app_key, app_secret, redirect_uri=None, response_type='code', domain='api.weibo.com', version='2', schema='https'):
        self.client_id = str(app_key)
        self.client_secret = str(app_secret)
        self.redirect_uri = redirect_uri
        self.response_type = response_type
        self.auth_url = '%s://%s/oauth2/' % (schema,domain)
        self.api_url = '%s://%s/%s/' % (schema, domain, version)
        self.access_token = None
        self.expires = 0.0
        self.get = HttpObject(self, _HTTP_GET)
        self.post = HttpObject(self, _HTTP_POST)
        self.upload = HttpObject(self, _HTTP_UPLOAD)
        #If parseJson is set to False, return the Http Response body as string
        self.parseJson = True
