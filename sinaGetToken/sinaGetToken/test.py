...
#     fr = open("./userpwd/4.txt","r")
#     lines = fr.readlines()
#     fw = open("./newFile/4.txt","a+")
#     for line in lines:
#         user,pwd = line.strip().split("\t")
#         print user,pwd
...
def start():
    obj = getToken()
    port = 10090
    local_ip = "192.168.34.1"#get_ip_address('eth0')
    print local_ip
...
import threading
thF = threading.Thread(target=start)

thS  = threading.Thread(target=token)
thS.start()
thF.start()